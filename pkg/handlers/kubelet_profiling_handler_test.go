...
func TestProfileKubelet(t *testing.T) {
	...
	testCases := []struct {
	...
	}{
		{
			...
			handlers: NewHandlers("abc", caCertPool, "/tmp", "/tmp/fakeSocket", "127.0.0.1", false),
		...
		},
		{
			...
			handlers: NewHandlers("abc", caCertPool, "non-existent-path", "/tmp/fakeSocket", "127.0.0.1", false),
			client: NewHTTPTestClient(func(req *http.Request) *http.Response {
				return &http.Response{
					StatusCode: 200,
					// Send response to be tested
